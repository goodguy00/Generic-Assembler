...
public class FileParser {
	 ...
	 * @param assemblyFile
	 */
	public FileParser(File specFile, File assemblyFile) {
	...
	private void scan(File assemblyFile, File specFile) {
		...
		if(!errorReport.isEmpty()){			
			...
			empty.add("Error in specification file, see \"" + Assemble.errFile().getName() + "\".");			
			Assembler.writeLinesToFile(Assemble.errFile(), errorReport);			
			Assembler.writeLinesToFile(Assemble.outFile(), empty);
			System.exit(0);
		}
	...
	private void scanAssemblyFile(File assemblyFile) {

		Scanner inputFile = null;

		try {
			inputFile = new Scanner(assemblyFile);
		} catch (FileNotFoundException e) {
			System.out.println("Assembly file \"" + assemblyFile.getName() + "\" not found.");
			System.exit(0);
		}
	...
	private void scanSpecFile(File specFile) {
		...
		try {
			inputFile = new Scanner(specFile);
			inputFile2 = new Scanner(specFile);
		} catch (FileNotFoundException e) {
			System.out.println("Specification file \"" + specFile.getName() + "\" not found.");
			System.exit(0);
		}
