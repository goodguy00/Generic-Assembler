...
public class Assembler {
	...
	private void assemble() {
		...
		writeLinesToFile(Assemble.outFile(), objectCode);
	}
		...
		for (String assemblyLine : data.getAssemblyCode()) {
			...
			if (assemblyLine.length() > 0) {
				try {					
					...
					writeLinesToFile(Assemble.errFile(), objectCode);
					writeLinesToFile(Assemble.outFile(), null);
					System.exit(0);
				}
			}
		}
	...
	private void analyseDataFirstPass(String assemblyLine) throws AssemblerException {
		...
		boolean legitIntDataLine = Pattern.matches(
				"[A-Za-z0-9_]+\\s+[0-9]+MAU\\s+[0-9]+", assemblyLine);

		boolean legitHexDataLine = Pattern.matches(
				"[A-Za-z0-9_]+\\s+[0-9]+MAU\\s+[0-9a-fA-F]+H", assemblyLine);

		boolean legitAsciiDataLine = Pattern.matches(
				"[A-Za-z0-9_]+\\s+.ascii\\s+\".+\"", assemblyLine);

		boolean legitUninitializedDataLine = Pattern.matches(
				"[A-Za-z0-9_]+\\s+[0-9]+MAU", assemblyLine);

		if (!legitIntDataLine && !legitAsciiDataLine && !legitUninitializedDataLine && !legitHexDataLine)
		...
		else if (legitIntDataLine || legitUninitializedDataLine || legitHexDataLine) {
	...
	private void populateDataSecondPass(String assemblyLine) throws AssemblerException {
		...
		if (legitAsciiDataLine) {
		...
		else if (legitHexDataLine) {
			String[] splitDataLine = assemblyLine.split("\\s+");
			binary = longHexToBinary(splitDataLine[2].substring(0, splitDataLine[2].length()-1));
			...
			if (binaryArray.size() > noOfMinAdrUnits)
				throw new AssemblerException("\"" + binary
						+ "\" exceeds expected bits.");
		}
	...
	private boolean analyseOperands(ArrayList<String> tokens,
		...
		for (String token : tokens) {
			...
			if (furtherTokenSplit.length > 1) { 				
				...
				return analyseOperands(furtherTokens, assemblyTokens,
			...
			// Single token
				...
				if (token.charAt(token.length() - 1) == '+') {
					...
					return analyseOperands(oneOrMoreExp, assemblyTokens,
		...
		for (String term : splitAssemblyOpTreeTerms) {
			...
			// AlphaNumeric
				...
				// Is register or mnemonic
					...
					if (!(splitAssemblyTerms[i]).matches("[a-zA-Z0-9_]+"))
//					if (!isAlpha(splitAssemblyTerms[i]))
	...
	public static String longHexToBinary(String hex) {
		String htb = "";
		hex = hex.substring(0, (hex.length() % 2 != 0) ? hex.length()-1 : hex.length());
		for(int i=0; i<hex.length(); i+=2) {
			htb += String.format("%8s", new BigInteger(hex.substring(i,i+2), 16).toString(2)).replace(' ', '0');
		}
		return htb;
	}
